...
void loop()
{
  ...
  {
    if (Serial1.available())
    { // ça remplit le tableau tant qu'on reçoit quelque chose par bluetooth
      String commande = Serial1.readStringUntil('\n');
      commande.trim();
...
void traiter_commande(char *commande)
{
  ...
  else if (lu == 1)
  {
    ...
    else if (strcmp(fonction, "P") == 0)
    { // Diagoale avant droite ***
      diagonaleAVD();
    }
    else if (strcmp(fonction, "N") == 0)
    { // Diagoale arriere droite ***
      diagonaleARD();
    }
    else if (strcmp(fonction, "A") == 0)
    { // Diagoale avant gauche ***
      diagonaleAVG();
    }
    else if (strcmp(fonction, "W") == 0)
    { // Diagoale arriere gauche ***
      diagonaleARG();
    }
...
bool stopObstacle(const int trigPin, const int echoPin, int seuilDetection)
